    ...
    const helper = async () => {
        if (schedule !== null) {
            ...
            for (let i = 0; i < schedule.length; i++) {
                ...
                const rfc3339Timestamp = convertToRFC3339("2023-11-06", timeStr);
                    ...
                    <Fade down>
                        <Box>
                            {dataArray.map((data, index) => {
                                const travelSegments = data.routes[0].legs[0].stepsOverview.multiModalSegments;
                                const steps = data.routes[0].legs[0].steps;
                                const initialWalk = (travelSegments && travelSegments[0].travelMode === "WALK") ? true : false;
                                const len = travelSegments && travelSegments.length;
                                let finalWalk = false;
                                if (len !== 1) {
                                    finalWalk = (travelSegments && travelSegments[len - 1].travelMode === "WALK") ? true : false;
                                }
                                let finalWalkStartIndex = -1;
                                let finalWalkEndIndex = -1;
                                if (finalWalk) {
                                    finalWalkStartIndex = travelSegments && travelSegments[len - 1].stepStartIndex;
                                    finalWalkEndIndex = travelSegments && travelSegments[len - 1].stepEndIndex;
                                }
